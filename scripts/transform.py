...
def load_json_files(prefix):
    """Week 9: Find and load raw JSON payloads matching prefix."""
...
def main():
    print("=" * 60)
    print("🚀 STARTING WEEKS 9 & 10 PANDAS ETL PIPELINE")
    print("=" * 60)
    os.makedirs(PROCESSED_DIR, exist_ok=True)

    # 1. Week 9: Load & Profile Raw Sources
    ...
    print("\n--- Week 9 Profiling: OpenWeatherMap Extract ---")
    if not df_owm.empty:
        print(df_owm.info())
        print(df_owm.head(2))
    else:
    ...
    # Generate Primary Key (observation_id)
    ...
    print(f"\n✅ Cleaned and transformed dataset saved to {OUTPUT_FILE}")
    print(f"📊 Final Record Count: {len(df_final)} rows")
    print("=" * 60)
